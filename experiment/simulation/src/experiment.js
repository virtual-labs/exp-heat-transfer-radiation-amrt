...
var stefan_boltzmann_constant,
  emissivity_float,
  radius_float,
  thickness_float,
  area_float;

var volt_needle_rotate_tp,
  ammeter_needle_rotate_tp,
  volt_needle_rotate_bp,
  ammeter_needle_rotate_bp;
...
var tp_count,
  bp_count,
  emissivity_blackbody,
  t4_chamber_temp,
  t1_to_t3_blackbody_temp,
  t5_to_t7_testplate_temp;

var vneedle_distance_interval_tp,
  aneedle_distance_interval_tp,
  vneedle_distance_interval_bp,
  aneedle_distance_interval_bp;

var power_array = [
  20, 34, 48, 62, 76, 90, 104, 118, 132, 146, 160, 174, 188, 202, 216, 230, 244,
  258, 272, 286, 300,
];
...
var current_array = [
  0.2, 0.309091, 0.4, 0.476923, 0.542857, 0.6, 0.65, 0.694118, 0.73333,
  0.768421, 0.8, 0.828571, 0.854545, 0.878261, 0.9, 0.92, 0.938462, 0.955556,
  0.971429, 0.986207, 1,
];
  ...
  return {
    ...
    link: function (scope, element, attrs, dialogs) {
      ...
      function handleComplete() {
        ...
        loadImages(
          queue.getResult("indicatorSelectorSwitchBase"),
          "indicatorSelectorSwitchBase",
          394,
          240,
          "",
          0,
        ...
        loadImages(
          queue.getResult("indicatorSelectorSwitch"),
          "indicatorSelectorSwitch",
          382,
          228,
          "",
          0,
        ...
        loadImages(
          queue.getResult("indicatorArrow"),
          "indicatorFwdArrow",
          420,
          229,
          "pointer",
        ...
        material_item_array = [
          ...
          "steelInner",
          "copperInner",
        ]; /** Array stores the collectionb of materials used for visibility */

        radiation_stage.getChildByName("motorOff").on("click", function () {
          /** Motor switch on functionality */
          scope.powerOnBtn(); /** Start the experiment with respect to the power on button */
          scope.$apply();
        });
        radiation_stage.getChildByName("motorOn").on("click", function () {
          /** Motor switch off functionality */
          powerOff(scope); /** Just pause the experiment */
          scope.$apply();
        });
        radiation_stage.getChildByName("switchOff").on("click", function () {
          /** Toggle switch turn to TP */
          ...
          if (tp_voltage_int == 0) {
            /** Rotation of ammeter and voltmeter needle on switch to TP */
            volt_needle_rotate_tp = tp_voltage_int / 6.67;
            ammeter_needle_rotate_tp = tp_voltage_int / 10;
          }
          rotateNeedlesTP(
            volt_needle_rotate_tp,
            ammeter_needle_rotate_tp,
          ); /** Rotation of needles */
        });
        radiation_stage.getChildByName("switchOn").on("click", function () {
          /** Toggle switch turn to BP */
          ...
          if (bp_voltage_int == 0) {
            /** Rotation of ammeter and voltmeter needle on switch to BP */
        ...
        radiation_stage
          .getChildByName("indicatorFwdArrow")
          .on("click", function () {
            /** Indicator forward rotation */
            if (indicator_text_count < 7) {
              /** Rotate the temperature indicator till the count reaches 7 */
              rotateTempIndicator(
                1,
                32,
              ); /** Counter increment by 1 and angle rotate by 32 */
              indicatorReadingFn(); /** The reading value display function */
            }
          });
        radiation_stage
          .getChildByName("indicatorBkdArrow")
          .on("click", function () {
            /** Indicator backward rotation */
      ...
      function translationLabels() {
        ...
        scope.material_array = [
          { material: _("Aluminium"), type: 0 },
          { material: _("Brass"), type: 1 },
          { material: _("Iron"), type: 2 },
          { material: _("Steel"), type: 3 },
          { material: _("Copper"), type: 4 },
        ];
        scope.$apply();
      }
    },
...
function setText(name, textX, textY, value, color, fontSize) {
  var _text = new createjs.Text(
    value,
    "bold " + fontSize + "em Tahoma, Geneva, sans-serif",
    color,
  );
  _text.x = textX;
  _text.y = textY;
...
function initialisationOfVariables() {
  ...
  radius_float =
    thickness_float =
    area_float =
      0; /** Initialize radius,thickness,area*/
  ...
  selected_material = 0;
  power_on_flag =
    toggle_flag = false; /** Initialize toggle and power on flag */
  tp_count = bp_count = 0;
...
function changeOption(scope) {
  ...
  for (var i = 0; i < 5; i++) {
    ...
    if (scope.Material == i) {
      ...
      radiation_stage.getChildByName(material_item_array[i + 5]).visible =
        false;
    }
  }
}
...
function diameterSliderFN(scope) {
  ...
  radiation_stage.getChildByName("commonDisc").scaleX =
    radiation_stage.getChildByName("aluminium").scaleX =
    radiation_stage.getChildByName("brass").scaleX =
    radiation_stage.getChildByName("iron").scaleX =
    radiation_stage.getChildByName("steel").scaleX =
    radiation_stage.getChildByName("copper").scaleX =
      _diameter_change_val;
}
...
function thicknessSliderFN(scope) {
  ...
  radiation_stage.getChildByName("commonDisc").y =
    radiation_stage.getChildByName("aluminium").y =
    radiation_stage.getChildByName("brass").y =
    radiation_stage.getChildByName("iron").y =
    radiation_stage.getChildByName("steel").y =
    radiation_stage.getChildByName("copper").y =
      541 - scope.thickness;
}
...
function temperatureSliderFN(scope) {
  scope.temperature =
    scope.Temperature; /** Change chamber temperature on slider change */
  temperature_val =
    parseInt(scope.temperature) +
    273; /** 273 is the kelvin value 0 degree= 273.15 K */
}
...
function startExperiment(scope, dialogs) {
  ...
  if (bp_voltage_int != tp_voltage_int) {
    ...
    timer_check = setInterval(
      expWatch,
      timer_interval,
    ); /** Check time equal to 20 minutes */
...
function checkTime() {
  ...
  if (
    (total_time <= 1200) &
    (radiation_stage.getChildByName("indicatorTimer").text <= resultant_temp)
  ) {
    if (total_time >= 1190) {
      //enable the indicator rotation just before 20 minutes
      radiation_stage.getChildByName("indicatorFwdArrow").mouseEnabled = true;
      radiation_stage.getChildByName("indicatorBkdArrow").mouseEnabled = true;
    }
  ...
  radiation_stage.getChildByName("tp_up_hit").on("click", function () {
    if (tp_count < 21) {
      ...
      if (tp_angle_float < 360) {
        tp_angle_float =
          tp_angle_float +
          17.5; /** Increment the tuner rotation. 17.5  for adjusting the tuner. */
        getRotation("tpTuner", tp_angle_float);
      }
      tp_count++;
      tpPowerVoltCurrentValues(
        tp_count,
      ); /** Power, voltage and current values of TP taken from the corresponding array */
      ...
      rotateNeedlesTP(
        volt_needle_rotate_tp,
        ammeter_needle_rotate_tp,
      ); /** Ammeter and voltmeter needle rotation */
    } else {
      radiation_stage.getChildByName("tp_up_hit").mouseEnabled = false;
      radiation_stage.getChildByName("tp_down_hit").mouseEnabled = true;
    }
...
function rotateNeedlesTP(val1, val2) {
  if (toggle_flag) {
    ...
    radiation_stage.getChildByName("ammeterReading").text =
      tp_current_int.toFixed(1);
  }
}
...
function rotateNeedlesBP(val1, val2) {
  if (!toggle_flag) {
    ...
    radiation_stage.getChildByName("ammeterReading").text =
      bp_current_int.toFixed(1);
  }
}

/** Function for the rotation of temperature indicator */
function rotateTempIndicator(incr, rotVal) {
  indicator_text_count =
    indicator_text_count + incr; /** Knob of the indicator */
  ...
  if (minute >= 20) {
    ...
    radiation_stage.getChildByName("indicatorTimer").text =
      resultant_temp.toFixed(2);
  }
}

/** The reading value display function */
function indicatorReadingFn() {
  radiation_stage.getChildByName("indicatorReading").text =
    "T" + indicator_text_count; /** Indicator count text box value */
}
...
function totalEmissivityCalculation() {
  emissivity_float =
    emissivity[
      selected_material
    ]; /** Select the emisitivity from the array given */
  ...
  area_float =
    2 * Math.PI * radius_float * thickness_float +
    Math.PI * Math.pow(radius_float, 2);
  /** (Tb4 - Tc4) K */
  var _blackplate_temp =
    bp_power_int /
    (emissivity_blackbody * stefan_boltzmann_constant * area_float);
  t4_chamber_temp =
    temperature_val * temperature_val * temperature_val * temperature_val;
  var _blackbody_temp = _blackplate_temp + t4_chamber_temp;
  t1_to_t3_blackbody_temp = Math.pow(_blackbody_temp, 1 / 4);

  /**  Tp4 - Tc4 K */
  var _temperature_of_plate =
    bp_power_int / (emissivity_float * stefan_boltzmann_constant * area_float);
  ...
  if (
    indicator_text_count == 1 ||
    indicator_text_count == 2 ||
    indicator_text_count == 3
  ) {
    /** The resultant value if indicator_text_count is in between 1 and 3 */
    resultant_temp = t1_to_t3_blackbody_temp;
  } else if (
    indicator_text_count == 5 ||
    indicator_text_count == 6 ||
    indicator_text_count == 7
  ) {
    /** The resultant value if indicator_text_count is in between 5 and 7 */
...
function displayIndicatorTimer() {
  if (power_on_flag) {
    if (tp_count == 0) {
      radiation_stage.getChildByName("indicatorTimer").text =
        initial_temp1.toFixed(2); /** Set initial temperature as 27 degree */
      radiation_stage.getChildByName("ammeterReading").text =
        0; /** Set the ammeter and voltmeter rotation as 0 */
      radiation_stage.getChildByName("voltmeterReading").text = 0;
    } else {
